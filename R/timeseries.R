...
#   tweets_tidy <- tweets_tidy %>% filter(year <= ymd_hms("2019-06-19 20:00:00"))
   ...
   animateTidyWords(tweets_tidy, output, filename = "animation_hashtag",mysubtitle = "Top 10 Hashtags em tuítes contendo termos 'Moro' ou 'Lula'", mycaption = "Dadoscope - https://medium.com/dadoscope")
} 
...
#	   geom_tile(aes(y = value/2, height = value, width = 0.9)) + 
	   ...
	   geom_text(aes(y = value, label = Value_lbl, vjust = 0.2, hjust = +1),size=8, col = "black") +
   ...
   if(output == "gif"){
      # For GIF
      animate(anim, length(formatted_tweets$year), fps = 15,  width = 977, height = 550,
              renderer = gifski_renderer("animation_lowres.gif"))
   }
   else{
      # For MP4
      animate(anim,length(formatted_tweets$year), fps = 15,  width = 3200, height = 1800,
...
#files<-system("ls tweets_Lula_201906[3]*.rds",intern=TRUE)
#tweets <- data.frame()
#for(f in files){
#  cat(f,sep="\n")
#  aux <- readRDS(f)
#  if(ncol(aux) >= 90){
##     tweets <- rbind(tweets, aux[,c("user_id","status_id","created_at","screen_name","text","hashtags","place_name","place_type","place_full_name","country","location","retweet_count","favorite_count")])
#     tweets <- rbind(tweets, aux[,c("created_at","hashtags")])
#  }
#}
...
saveRDS(tweets, file = "subset_tweets_LavaJato_0907.rds")
